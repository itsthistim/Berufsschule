...
if (isset($_POST['save'])) {
    ...
    $sql_street = "INSERT INTO strasse (str_name) VALUES (?)";
    $sql_city = "INSERT INTO strasse_ort_plz values (?, ?)";


    try {
        // save street
        $stmt = $conn->prepare($sql_street);
        $stmt->execute([$street_name]);
        $street_id = $conn->lastInsertId();

        // save city
        $stmt = $conn->prepare($sql_city);
        $stmt->execute([$street_id, $city]);

        echo '<br /><br /><br /><br /><div class="alert alert-success" role="alert">
        <h4 class="alert-heading">Well done!</h4>
        <p>You successfully inserted a new street.</p>
        </div>';
        ...
        <div class="formbold-form-wrapper">
            ...
            <form method="POST">
                ...
                <div class="formbold-input-flex">
                    <div>
                        <label for="street_name" class="formbold-form-label">
                            Street name
                        </label>
                    ...
                    <div>
                        ...
                        <?php
                        $sql = 'SELECT orpl_id, plz_nr as "ZIP", ort_name as "City" from ort_plz natural join (ort, plz) order by plz_nr ORDER BY 1';
                        $stmt = $conn->prepare($sql);
                        $stmt->execute();

                        echo '<select name="city" id="city" class="formbold-form-input">';
                        echo '<option value="" disabled selected>Select your city</option>';
